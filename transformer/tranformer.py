...
if not os.path.isdir("mobilevit_xxs"):
...
def representative_data_gen():
    ...
    for x in val_dataset:      
        #data= tf.image.resize_with_crop_or_pad(x[0], 256,256)
        #tf.cast(data,tf.uint8)
        #data = tf.reshape(data, shape=[-1, 224, 244, 3])
        #data = preprocess(data)
        #a = np.array([data])
        img = tf.data.Dataset.from_tensor_slices(x[0]).batch(64)
        #print(f'lol {data.shape}')
        for i in img.take(1):                    
            yield [i]
# Convert to TFLite. This form of quantization is called
# post-training dynamic-range quantization in TFLite.
converter_quant = tf.lite.TFLiteConverter.from_saved_model("mobilevit_xxs")
